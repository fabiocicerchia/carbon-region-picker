...
```mermaid
graph LR
  carbon_region_picker["carbon_region_picker<br/><small>Python · 1 mod · 310 loc</small>"]
```
...
1 types declared: 0 inheritance and 0 composition relationships between types defined in this tree. Relationships to types declared elsewhere are omitted rather than guessed, so this is a lower bound. 1 types were read with a real parser; the rest come from declaration syntax, which is reliable for the declaration and weaker for the member lists.
...
**carbon_region_picker.py**

```
carbon_region_picker  (Python)
```
...
|---|---|---:|---:|---:|---:|---:|
| `carbon_region_picker` | Python | 1 | 310 | 0 | 0 | 0.0 |
...
|---|---:|---:|---|
| `requests` | 1 | 1 | carbon_region_picker.py:150 |

7 standard-library modules imported; most used: `argparse` (1), `dataclasses` (1), `json` (1), `os` (1), `socket` (1), `sys` (1), `time` (1).
...
|---|---:|---:|---|
| `carbon_region_picker.py` | 526 | 310 | Python |

## Public surface

<details><summary><code>carbon_region_picker</code> — 18 exported</summary>


`carbon_region_picker`

- class RankedRegion:85
- const CLEAN_GRID_GCO2_KWH:22
- const ENDPOINTS:78
- const EXIT_USAGE:20
- const HTTPS_PORT:17
- const REGIONS:36
- const VANTAGE:73
- def best_forecast_slot:182
- def build_parser:221
- def emit:266
- def fetch_forecast:176
- def fetch_live:161
- def main:278
- def measure_all:133
- def measure_latency_ms:119
- def rank:94
- def render:189
- def resolve_token:259
